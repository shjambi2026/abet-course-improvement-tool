...
def _format_so_distribution(assessment):
    distribution = assessment.get("so_distribution", {})
    if distribution:
        items = sorted(distribution.items(), key=lambda item: int(str(item[0]).replace("SO", "")))
        return ", ".join(f"{so} ({value:g}%)" for so, value in items)
    sos = assessment.get("mapped_sos", assessment.get("assessed_sos", assessment.get("sos", [])))
    return _format_list(_sort_ids(sos, "SO"))
...
def generate_coordination_mom(
    ...
    **kwargs,
    ...
    attendee_items = [line.strip() for line in str(attendees or "").splitlines() if line.strip()] or [""]
    topic_items = [line.strip() for line in str(topics_discussed or kwargs.get("discussion_notes", "")).splitlines() if line.strip()] or [""]
...
def generate_assessment_cover_sheet(
    ...
    **kwargs,
    ...
    distribution = assessment.get("so_distribution", {})
    if distribution:
        for so, percent in sorted(distribution.items(), key=lambda item: int(str(item[0]).replace("SO", ""))):
    ...
    if so_rows:
        total_so_marks = sum(float(str(row[1]).replace(",", "")) for row in so_rows if str(row[1]).strip())
        lines.append(_markdown_table(["Student Outcome", "Maximum Marks"], so_rows + [["Total (SO)", f"{total_so_marks:g}"]]))
    else:
...
def generate_eos_report(
    ...
    **kwargs,
    ...
    if combined_so_attainment and combined_so_attainment.get("summary"):
        ...
        lines.append(_markdown_table(["Student outcome", "Students assessed", "Students attained", "Attainment", "Target met"], rows))
        lines.append("")
    ...
    lines.append(assessment_based_evaluation or "No assessment-based evaluation was entered.")
    lines.append("")
    lines.append("## Subjective Evaluation")
    lines.append(subjective_evaluation or "No subjective evaluation was entered.")
    lines.append("")
    lines.append("## Course Improvement")
    lines.append(course_improvement or "No course improvement actions were entered.")
    lines.append("")
